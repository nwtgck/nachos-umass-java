...
public class AutoGrader {
   ...
   * <tt>KThread.createIdleThread()</tt> <i>must</i> call this method before forking the idle
   * thread.
   *
   ...
   * <tt>KThread.ready()</tt> <i>must</i> call this method before returning.
   *
  ...
  /**
   * Notify the autograder that the specified thread is now running. <tt>KThread.restoreState()</tt>
   * <i>must</i> call this method before returning.
   *
  ...
  /**
   * Notify the autograder that the current thread has finished. <tt>KThread.finish()</tt>
   * <i>must</i> call this method before putting the thread to sleep and scheduling its TCB to be
   * destroyed.
   */
  ...
  /**
   * Notify the autograder that a COFF loader is being constructed for the specified file. The
   * autograder can use this to provide its own COFF loader, or return <tt>null</tt> to use the
   * default loader.
   *
   * @param file the executable file being loaded.
   * @return a loader to use in loading the file, or <tt>null</tt> to use the default.
   */
  ...
  /**
   * Request permission to send a packet. The autograder can use this to drop packets very
   * selectively.
   *
   * @param privilege proves the authenticity of this call.
