...
public final class ElevatorGui extends Frame {
  private static final int w = 90, h = 75;
  ...
  private void paintRiders(Graphics g, int x, int y, int w, int h, int n) {
    ...
    for (int j = 0; j < yn; j++) {
      for (int i = 0; i < xn; i++) {
        if (n-- > 0) paintRider(g, x0 + i * t, y0 - j * t, r);
      }
    }
  }
  ...
  private class Floor extends Canvas {
    ...
    private void paintButtons(Graphics g) {
      ...
      if (floor < numFloors - 1) {
        if (upSet) g.fillOval(xc - r, yc1 - r, 2 * r, 2 * r);
        else g.drawOval(xc - r, yc1 - r, 2 * r, 2 * r);
      }

      if (floor > 0) {
        if (downSet) g.fillOval(xc - r, yc2 - r, 2 * r, 2 * r);
        else g.drawOval(xc - r, yc2 - r, 2 * r, 2 * r);
      }
    }
  }
  ...
  private class ElevatorShaft extends Canvas {
    ElevatorShaft(int elevator) {
      ...
      for (int i = 0; i < numFloors; i++) floorsSet[i] = false;
    ...
    public void paint(Graphics g) {
      ...
      if (g.hitClip(0, 0, s, h * numFloors)) {
        ...
        for (int y = 0; y < h * numFloors - s; y += s) g.drawLine(0, y, s - 1, y + s - 1);
      }

      if (g.hitClip(w - s, 0, s, h * numFloors)) {
        g.drawLine(w - s, 0, w - s, h * numFloors);
        g.drawLine(w - 1, 0, w - 1, h * numFloors);
        for (int y = 0; y < h * numFloors - s; y += s) g.drawLine(w - s, y, w - 1, y + s - 1);
      }
      ...
      if (doorsOpen) {
        ...
        for (int x = 0; x < e.width; x += 2 * s) g.drawLine(e.x + x, e.y, e.x + x, e.y + e.height);
      }
      ...
      // draw arrows
      if (direction == ElevatorBank.dirUp) g.fillPolygon(xUp, yUp, 3);
      else g.drawPolygon(xUp, yUp, 3);

      if (direction == ElevatorBank.dirDown) g.fillPolygon(xDown, yDown, 3);
      else g.drawPolygon(xDown, yDown, 3);
    }
