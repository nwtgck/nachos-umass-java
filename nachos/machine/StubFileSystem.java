...
/**
 * This class implements a file system that redirects all requests to the host operating system's
 * file system.
 */
public class StubFileSystem implements FileSystem {
  ...
  private class StubOpenFile extends OpenFileWithPosition {
    StubOpenFile(final String name, final boolean truncate) throws IOException {
      ...
      if (openCount == maxOpenFiles) throw new IOException();
  ...
  private static boolean checkName(String name) {
    ...
    for (int i = 0; i < chars.length; i++) {
      if (chars[i] < 0 || chars[i] >= allowedFileNameCharacters.length) return false;
      if (!allowedFileNameCharacters[(int) chars[i]]) return false;
    }
    return true;
  }
  ...
  private static void reject(char first, char last) {
    for (char c = first; c <= last; c++) allowedFileNameCharacters[c] = false;
  }

  private static void allow(char first, char last) {
    for (char c = first; c <= last; c++) allowedFileNameCharacters[c] = true;
  }
