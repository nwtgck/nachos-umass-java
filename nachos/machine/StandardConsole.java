...
public class StandardConsole implements SerialConsole {
  ...
  public final void setInterruptHandlers(
  ...
  protected int in() {
    try {
      if (System.in.available() <= 0) return -1;
  ...
  private int translateCharacter(int c) {
    ...
    else if (c == 0x04 || c == 0x08 || c == 0x19 || c == 0x1B || c == 0x7F) return '\b';
    // if normal ASCII range, nothing to do
    else if (c >= 0x20) return c;
    // newline characters
    else if (c == 0x0A || c == 0x0D) return '\n';
  ...
  private void receiveInterrupt() {
    ...
    if (incomingKey == -1) {
      ...
      if (receiveInterruptHandler != null) receiveInterruptHandler.run();
    }
  }
  ...
  public final void writeByte(int value) {
    if (outgoingKey == -1) scheduleSendInterrupt();
