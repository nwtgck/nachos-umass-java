...
/**
 * The <tt>Interrupt</tt> class emulates low-level interrupt hardware. The hardware provides a
 * method (<tt>setStatus()</tt>) to enable or disable interrupts.
 *
 * <p>In order to emulate the hardware, we need to keep track of all pending interrupts the hardware
 * devices would cause, and when they are supposed to occur.
 *
 * <p>This module also keeps track of simulated time. Time advances only when the following occur:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>As a result, unlike real hardware, interrupts (including time-slice context switches) cannot
 * occur just anywhere in the code where interrupts are enabled, but rather only at those places in
 * the code where simulated time advances (so that it becomes time for the hardware simulation to
 * invoke an interrupt handler).
 *
 * <p>This means that incorrectly synchronized code may work fine on this hardware simulation (even
 * with randomized time slices), but it wouldn't work on real hardware. But even though Nachos can't
 * always detect when your program would fail in real life, you should still write properly
 * synchronized code.
 */
public final class Interrupt {
  ...
  /**
   * Disable interrupts and return the old interrupt state. This method has the same effect as
   * <tt>setStatus(false)</tt>.
   *
  ...
  /**
   * Restore interrupts to the specified status. This method has the same effect as
   * <tt>setStatus(<i>status</i>)</tt>.
   *
   ...
   * the previous status. If the interrupt status changes from disabled to enabled, the simulated
   * time is advanced.
   *
   * @param status <tt>true</tt> to enable interrupts.
   * @return <tt>true</tt> if interrupts were enabled.
  ...
  public boolean setStatus(boolean status) {
    ...
    if (oldStatus == false && status == true) tick(true);
  ...
  private void tick(boolean inKernelMode) {
    ...
    if (Lib.test(dbgInt)) System.out.println("== Tick " + stats.totalTicks + " ==");
  ...
  private void checkIfDue() {
    ...
    if (Lib.test(dbgInt)) print();

    if (pending.isEmpty()) return;

    if (((PendingInterrupt) pending.first()).time > time) return;
    ...
    while (!pending.isEmpty() && ((PendingInterrupt) pending.first()).time <= time) {
      ...
      if (privilege.processor != null) privilege.processor.flushPipe();
  ...
  private void print() {
    System.out.println(
        "Time: " + privilege.stats.totalTicks + ", interrupts " + (enabled ? "on" : "off"));
  ...
  private class PendingInterrupt implements Comparable {
    ...
    public int compareTo(Object o) {
      ...
      if (time < toOccur.time) return -1;
      else if (time > toOccur.time) return 1;
      else if (id < toOccur.id) return -1;
      else if (id > toOccur.id) return 1;
      else return 0;
    }
