 ...
 * instruction set. Specifically, the processor lacks all coprocessor support, and can only execute
 * in user mode. Address translation information is accessed via the API. The API also allows a
 * kernel to set an exception handler to be called on any user mode exception.
 *
 * <p>The <tt>Processor</tt> API is re-entrant, so a single simulated processor can be shared by
 * multiple user threads.
 *
 * <p>An instance of a <tt>Processor</tt> also includes pages of physical memory accessible to user
 * programs, the size of which is fixed by the constructor.
 */
public final class Processor {
   ...
   *
   * <p>When the exception handler is called, interrupts will be enabled, and the CPU cause register
   * will specify the cause of the exception (see the <tt>exception<i>*</i></tt> constants).
   *
   ...
   *
   * <p>If <tt>false</tt>, this processor directly supports single-level paging; use
   * <tt>setPageTable()</tt>.
   *
   * <p>If <tt>true</tt>, this processor has a software-managed TLB; use <tt>getTLBSize()</tt>,
   * <tt>readTLBEntry()</tt>, and <tt>writeTLBEntry()</tt>.
   *
   * <p>Using a method associated with the wrong address translation mechanism will result in an
   * assertion failure.
   *
  ...
  /**
   * Set the page table pointer. All further address translations will use the specified page table.
   * The size of the current address space will be determined from the length of the page table
   * array.
   *
   ...
   *
   * <p>The TLB is fully associative, so the location of an entry within the TLB does not affect
   * anything.
   *
   * @param number the index into the TLB.
  ...
  /**
   * Return a reference to the physical memory array. The size of this array is <tt>pageSize *
   * getNumPhysPages()</tt>.
   *
   ...
   *
   * @param page the page number. Must be between <tt>0</tt> and <tt>(2<sup>32</sup> / pageSize) -
   *     1</tt>.
  ...
  /**
   * Translate a virtual address into a physical address, using either a page table or a TLB. Check
   * for alignment, make sure the virtual page is valid, make sure a read-only page is not being
   * written, make sure the resulting physical page is valid, and then return the resulting physical
   * address.
   *
  ...
  private int translate(int vaddr, int size, boolean writing) throws MipsException {
    if (Lib.test(dbgProcessor))
      System.out.println(
          "\ttranslate vaddr=0x" + Lib.toHexString(vaddr) + (writing ? ", write" : ", read..."));
  ...
  /**
   * Read </i>size</i> (1, 2, or 4) bytes of virtual memory at <i>vaddr</i>, and return the result.
   *
  ...
  /**
   * Write <i>value</i> to </i>size</i> (1, 2, or 4) bytes of virtual memory starting at
   * <i>vaddr</i>.
   *
   ...
   * @param nextLoadMask the mask specifying which bits in the new target are to be overwritten. If
   *     a bit in <tt>nextLoadMask</tt> is 0, then the corresponding bit of register
   ...
   *
   * <p>Transfer the contents of the nextPC register into the PC register, and then add 4 to the
   * value in the nextPC register. Same as <tt>advancePC(readRegister(regNextPC)+4)</tt>.
   *
   * <p>Use after handling a syscall exception so that the processor will move on to the next
   * instruction.
   */
  ...
  /**
   * Transfer the contents of the nextPC register into the PC register, and then write the nextPC
   * register.
   *
  ...
  /**
   * Either an associative or direct-mapped set of translation entries, depending on whether there
   * is a TLB.
   */
  ...
  private class MipsException extends Exception {
    ...
    public void handle() {
      ...
      if (!Machine.autoGrader().exceptionHandler(privilege)) return;
  ...
  private class Instruction {
    ...
    private void decode() {
      ...
      if (test(Mips.SIZEB)) size = 1;
      else if (test(Mips.SIZEH)) size = 2;
      else if (test(Mips.SIZEW)) size = 4;
      else size = 0;
      ...
      // get dstReg
      if (test(Mips.DSTRA)) dstReg = regRA;
      else if (format == Mips.IFMT) dstReg = rt;
      else if (format == Mips.RFMT) dstReg = rd;
      else dstReg = -1;

      // get jtarget
      if (format == Mips.RFMT) jtarget = registers[rs];
      else if (format == Mips.IFMT) jtarget = registers[regNextPC] + (imm << 2);
      else if (format == Mips.JFMT) jtarget = (registers[regNextPC] & 0xF0000000) | (target << 2);
      else jtarget = -1;
      ...
      // get src1
      if (test(Mips.SRC1SH)) src1 = sh;
      else src1 = registers[rs];

      // get src2
      if (test(Mips.SRC2IMM)) src2 = imm;
      else src2 = registers[rt];
      ...
      if (Lib.test(dbgDisassemble) || Lib.test(dbgFullDisassemble)) print();
    }
    ...
    private void print() {
      ...
      if (operation == Mips.INVALID) {
        System.out.print(
            "invalid: op="
                + Lib.toHexString(op, 2)
                + " rs="
                + Lib.toHexString(rs, 2)
                + " rt="
                + Lib.toHexString(rt, 2)
                + " rd="
                + Lib.toHexString(rd, 2)
                + " sh="
                + Lib.toHexString(sh, 2)
                + " func="
      ...
      switch (operation) {
        ...
        case Mips.DIV:
          try {
            ...
            if (registers[regLo] * src2 + registers[regHi] != src1) throw new ArithmeticException();
        ...
        case Mips.LOAD:
          ...
          if (!test(Mips.UNSIGNED)) dst = Lib.extend(value, 0, size * 8);
          else dst = value;
    ...
    private void writeBack() throws MipsException {
      ...
      if (test(Mips.DELAYEDLOAD)) delayedLoad(dstReg, (int) dst, mask);
      else finishLoad();
      ...
      if ((test(Mips.DST) || test(Mips.DELAYEDLOAD)) && dstReg != 0) {
        if (Lib.test(dbgFullDisassemble)) {
          ...
          if (test(Mips.DELAYEDLOAD)) System.out.print(" (delayed load)");
        }
      }
  ...
  private static class Mips {
    ...
    static final int DST = 0x00000001,
        DSTRA = 0x00000002,
        OVERFLOW = 0x00000004,
        SRC1SH = 0x00000008,
        SRC2IMM = 0x00000010,
