...
public final class ElevatorBank implements Runnable {
  ...
  /**
   * Initialize this elevator bank with the specified number of elevators and the specified number
   * of floors. The software elevator controller must also be specified. This elevator must not
   * already be running a simulation.
   *
  ...
  public void init(int numElevators, int numFloors, ElevatorControllerInterface controller) {
    ...
    for (int i = 0; i < numElevators; i++) elevators[i] = new ElevatorState(0);
  ...
  /**
   * Run a simulation. Initialize all elevators and riders, and then fork threads to each of their
   * <tt>run()</tt> methods. Return when the simulation is finished.
   */
  public void run() {
    Lib.assertTrue(!simulationStarted);
    ...
    for (int i = 0; i < numRiders; i++) riders[i].initialize();
    manager.initialize();

    for (int i = 0; i < numRiders; i++) riders[i].run();
    manager.run();

    for (int i = 0; i < numRiders; i++) riders[i].join();
    manager.join();
  ...
  private void initGui() {
    ...
    for (int floor = 0; floor < numFloors; floor++) numRidersPerFloor[floor] = 0;

    for (int rider = 0; rider < numRiders; rider++) numRidersPerFloor[riders[rider].floor]++;
  ...
  private class ElevatorManager implements ElevatorControls {
    ...
    public void openDoors(int elevator) {
      ...
      if (gui != null) {
        if (elevators[elevator].direction == dirUp) gui.clearUpButton(elevators[elevator].floor);
    ...
    public void closeDoors(int elevator) {
      ...
      if (gui != null) gui.closeDoors(elevator);
    }
    ...
    public boolean moveTo(int floor, int elevator) {
      ...
      if (!elevators[elevator].moveTo(floor)) return false;
    ...
    public void setDirectionDisplay(int elevator, int direction) {
      ...
      if (gui != null) {
        if (elevators[elevator].doorsOpen) {
          if (direction == dirUp) gui.clearUpButton(elevators[elevator].floor);
          else if (direction == dirDown) gui.clearDownButton(elevators[elevator].floor);
        }
    ...
    public ElevatorEvent getNextEvent() {
      ...
      else return (ElevatorEvent) events.removeFirst();
    }
    ...
    void interrupt() {
      for (int i = 0; i < numElevators; i++) {
        if (elevators[i].atNextFloor()) {
          if (gui != null) gui.elevatorMoved(elevators[i].floor, i);
      ...
      if (!finished && !events.isEmpty() && handler != null) handler.run();
    }
  ...
  private class ElevatorState {
    ...
    boolean moveTo(int newDestination) {
      ...
      if (!moving) {
        ...
        if (floor == newDestination) return false;
        ...
        if ((destination > floor && newDestination <= floor)
            || (destination < floor && newDestination >= floor)) return false;
    ...
    boolean enter(RiderState rider, int onFloor) {
      ...
      if (!doorsOpen || moving || onFloor != floor || riders.size() == maxRiders) return false;
    ...
    boolean exit(RiderState rider, int onFloor) {
      ...
      if (!doorsOpen || moving || onFloor != floor) return false;
    ...
    boolean atNextFloor() {
      if (!moving || Machine.timer().getTime() < nextETA) return false;
    ...
    boolean atDestination() {
      if (!moving || destination != floor) return false;
  ...
  private class RiderState implements RiderControls {
    ...
    public int[] getFloors() {
      ...
      for (int i = 0; i < array.length; i++) array[i] = ((Integer) floors.elementAt(i)).intValue();
    ...
    public RiderEvent getNextEvent() {
      if (events.isEmpty()) return null;
      else return (RiderEvent) events.removeFirst();
    }

    public boolean pressDirectionButton(boolean up) {
      if (up) return pressUpButton();
      else return pressDownButton();
    }
    ...
    public boolean pressUpButton() {
      ...
      for (int elevator = 0; elevator < numElevators; elevator++) {
        if (elevators[elevator].doorsOpen
            ...
            && elevators[elevator].floor == floor) return false;
      }

      manager.postEvent(ElevatorEvent.eventUpButtonPressed, floor, -1, true);

      if (gui != null) gui.pressUpButton(floor);
    ...
    public boolean pressDownButton() {
      ...
      if (gui != null) gui.pressDownButton(floor);
    ...
    public boolean enterElevator(int elevator) {
      ...
      if (!elevators[elevator].enter(this, floor)) return false;

      if (gui != null) gui.enterElevator(floor, elevator);
    ...
    public boolean pressFloorButton(int floor) {
      ...
      if (elevators[elevator].doorsOpen && elevators[elevator].floor == floor) return false;

      manager.postEvent(ElevatorEvent.eventFloorButtonPressed, floor, elevator, true);

      if (gui != null) gui.pressFloorButton(floor, elevator);
    ...
    public boolean exitElevator(int floor) {
      ...
      if (!elevators[elevator].exit(this, floor)) return false;

      inElevator = false;
      floors.add(new Integer(floor));

      if (gui != null) gui.exitElevator(floor, elevator);
    ...
    public void finish() {
      ...
      for (int i = 0; i < floors.length; i++) Lib.assertTrue(floors[i] == stops[i]);
