...
public final class Lib {
  ...
  /**
   * Return a random integer between 0 and <i>range - 1</i>. Must not be called before
   * <tt>seedRandom()</tt> seeds the random number generator.
   *
   * @param range a positive value specifying the number of possible return values.
  ...
  /**
   * Asserts that <i>expression</i> is <tt>true</tt>. If not, then Nachos exits with an error
   * message.
   *
   * @param expression the expression to assert.
   */
  public static void assertTrue(boolean expression) {
    if (!expression) throw new AssertionFailureError();
  }

  /**
   * Asserts that <i>expression</i> is <tt>true</tt>. If not, then Nachos exits with the specified
   * error message.
   *
   * @param expression the expression to assert.
  ...
  public static void assertTrue(boolean expression, String message) {
    if (!expression) throw new AssertionFailureError(message);
  }
  ...
  /**
   * Asserts that this call is never made, with the specified error messsage. Same as
   * <tt>assertTrue(false, message)</tt>.
   *
   * @param message the error message.
   */
  ...
  /**
   * Print <i>message</i> if <i>flag</i> was enabled on the command line. To specify which flags to
   * enable, use the -d command line option. For example, to enable flags a, c, and e, do the
   * following:
   *
   * <p>
   *
   ...
   *
   * <p>Nachos uses several debugging flags already, but you are encouraged to add your own.
   *
   * @param flag the debug flag that must be set to print this message.
   * @param message the debug message.
   */
  public static void debug(char flag, String message) {
    if (test(flag)) System.out.println(message);
  }
  ...
  public static boolean test(char flag) {
    ...
    else if (debugFlags[(int) '+']) return true;
    else if (flag >= 0 && flag < 0x80 && debugFlags[(int) flag]) return true;
    else return false;
  }
  ...
  public static void enableDebugFlags(String flagsString) {
    if (debugFlags == null) debugFlags = new boolean[0x80];

    char[] newFlags = flagsString.toCharArray();
    for (int i = 0; i < newFlags.length; i++) {
      char c = newFlags[i];
      if (c >= 0 && c < 0x80) debugFlags[(int) c] = true;
    }
  }
  ...
  /**
   * Read a file, verifying that the requested number of bytes is read, and verifying that the read
   * operation took a non-zero amount of time.
   *
   ...
   */
  public static void strictReadFile(
      OpenFile file, int position, byte[] buf, int offset, int length) {
   ...
   * @return an array containing the contents of the entire file, or <tt>null</tt> if an error
   *     occurred.
   */
  ...
  /**
   * Convert an int into its little-endian byte string representation, and return an array
   * containing it.
   *
   * @param value the value to convert.
  ...
  public static short bytesToShort(byte[] array, int offset) {
    return (short)
        ((((short) array[offset + 0] & 0xFF) << 0) | (((short) array[offset + 1] & 0xFF) << 8));
  }
   ...
   * @return a string containing the specified bytes, up to and not including the null-terminator
   *     (if present).
   */
  ...
  public static int extract(int bits, int lowest, int size) {
    if (size == 32) return (bits >> lowest);
    else return ((bits >> lowest) & ((1 << size) - 1));
  }
  ...
  public static long extract(long bits, int lowest, int size) {
    if (size == 64) return (bits >> lowest);
    else return ((bits >> lowest) & ((1L << size) - 1));
  }
  ...
  /**
   * Creates a padded upper-case string representation of the integer argument in base 16, padding
   * to at most the specified number of digits.
   *
   * @param i an integer.
  ...
  /**
   * Load and return the named class, or return <tt>null</tt> if the class could not be loaded.
   *
  ...
  /**
   * Create and return a new instance of the named class, using the constructor that takes no
   * arguments.
   *
  ...
  /**
   * Verifies that the specified class is public and not abstract, and that a constructor with the
   * specified signature exists and is public.
   *
  ...
  public static void checkConstructor(Class cls, Class[] parameterTypes) {
    try {
      Lib.assertTrue(
          Modifier.isPublic(cls.getModifiers()) && !Modifier.isAbstract(cls.getModifiers()));
  ...
  /**
   * Verifies that the specified class is public, and that a non-static method with the specified
   * name and signature exists, is public, and returns the specified type.
   *
  ...
  public static void checkMethod(
      Class cls, String methodName, Class[] parameterTypes, Class returnType) {
    try {
  ...
  /**
   * Verifies that the specified class is public, and that a static method with the specified name
   * and signature exists, is public, and returns the specified type.
   *
  ...
  /**
   * Verifies that the specified class is public, and that a non-static field with the specified
   * name and type exists, is public, and is not final.
   *
  ...
  /**
   * Verifies that the specified class is public, and that a static field with the specified name
   * and type exists and is public.
   *
