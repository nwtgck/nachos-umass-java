...
/** Tests the <tt>ElevatorBank</tt> module, using a single elevator and a single rider. */
public final class ElevatorTest {
  ...
  private class ElevatorController implements ElevatorControllerInterface {
    ...
    public void run() {
      ...
      e = getNextEvent();
      Lib.assertTrue(
          e.event == ElevatorEvent.eventElevatorArrived && e.floor == 1 && e.elevator == 0);
    ...
    private ElevatorEvent getNextEvent() {
      ElevatorEvent event;
      while (true) {
        if ((event = controls.getNextEvent()) != null) break;
