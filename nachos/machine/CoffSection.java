...
public class CoffSection {
  ...
  protected CoffSection(
      Coff coff, String name, boolean executable, boolean readOnly, int numPages, int firstVPN) {
    this.coff = coff;
    this.name = name;
  ...
  /**
   * Test whether this section is initialized. Loading a page from an initialized section requires a
   * disk access, while loading a page from an uninitialized section requires only zero-filling the
   * page.
   *
   * @return <tt>true</tt> if this section contains initialized data in the executable.
   */
  ...
  public void loadPage(int spn, int ppn) {
    ...
    if (!initialized) initlen = 0;
    else if (spn == numPages - 1)
      /**
       * initlen = size % pageSize; Bug identified by Steven Schlansker 3/20/08 Bug fix by Michael
       * Rauser
       */
      initlen = (size == pageSize) ? pageSize : (size % pageSize);
    else initlen = pageSize;

    if (initlen > 0) Lib.strictReadFile(file, faddr, memory, paddr, initlen);
