...
public class Coff {
   ...
   *
   * <ol>
   *   <li>If the constructor returns successfully, the file becomes the property of this loader,
   *       and should not be accessed any further.
   *   <li>The autograder expects this loader class to be used. Do not load sections through any
   *       other mechanism.
   *   <li>This loader will verify that the file is backed by a file system, by asserting that
   *       read() operations take non-zero simulated time to complete. Do not supply a file backed
   *       by a simulated cache (the primary purpose of this restriction is to prevent sections from
   *       being loaded instantaneously while handling page faults).
   * </ol>
   *
  ...
  /**
   * Return an object that can be used to access the specified section. Valid section numbers
   * include <tt>0</tt> through <tt>getNumSections() - 1</tt>.
   *
  ...
  /**
   * Return the program entry point. This is the value that to which the PC register should be
   * initialized to before running the program.
   *
