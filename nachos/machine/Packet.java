...
public class Packet {
  ...
  public Packet(int dstLink, int srcLink, byte[] contents) throws MalformedPacketException {
    ...
    if (dstLink < 0
        || dstLink >= linkAddressLimit
        || srcLink < 0
        || srcLink >= linkAddressLimit
        || contents.length > maxContentsLength) throw new MalformedPacketException();
  ...
  public Packet(byte[] packetBytes) throws MalformedPacketException {
    ...
    if (packetBytes.length < headerLength
        || packetBytes[0] != NetworkLink.networkID
        || packetBytes[1] < 0
        || packetBytes[1] >= linkAddressLimit
        || packetBytes[2] < 0
        || packetBytes[2] >= linkAddressLimit
        || packetBytes[3] < 0
        || packetBytes[3] > packetBytes.length - 4) throw new MalformedPacketException();
  ...
  /**
   * The maximum number of content bytes (not including the header). Note that this is just
   * <tt>maxPacketLength - headerLength</tt>.
   */
  public static final int maxContentsLength = maxPacketLength - headerLength;

  /**
   * The upper limit on Nachos link addresses. All link addresses fall between <tt>0</tt> and
   * <tt>linkAddressLimit - 1</tt>.
   */
