...
/**
 * A KThread is a thread that can be used to execute Nachos kernel code. Nachos allows multiple
 * threads to run concurrently.
 *
 * <p>To create a new thread of execution, first declare a class that implements the
 * <tt>Runnable</tt> interface. That class then implements the <tt>run</tt> method. An instance of
 * the class can then be allocated, passed as an argument when creating <tt>KThread</tt>, and
 * forked. For example, a thread that computes pi could be written as follows:
 *
 * <p>
 *
 * <blockquote>
...
public class KThread {
  ...
  /**
   * Allocate a new <tt>KThread</tt>. If this is the first <tt>KThread</tt>, create an idle thread
   * as well.
   */
  public KThread() {
  ...
  /**
   * Get the full name of this thread. This includes its name along with its numerical ID. This name
  ...
  /**
   * The status of this thread. A thread can either be new (not yet forked), ready (on the ready
   * queue but not running), running, or blocked (not on the ready queue and not running).
   */
