 ...
 * When a thread begins waiting for access, it is appended to the end of a list. The next thread to
 * receive access is always the first thread in the list. This causes access to be given on a
...
public class RoundRobinScheduler extends Scheduler {
  ...
  private class FifoQueue extends ThreadQueue {
     ...
     *
     * @return the first thread on the queue, or <tt>null</tt> if the queue is empty.
     */
     ...
     * <tt>nextThread()</tt>. Assert that no threads are waiting for access.
     */
