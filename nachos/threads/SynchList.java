...
public class SynchList {
  ...
  /**
   * Add the specified object to the end of the queue. If another thread is waiting in
   * <tt>removeFirst()</tt>, it is woken up.
   *
  ...
  /**
   * Remove an object from the front of the queue, blocking until the queue is non-empty if
   * necessary.
   *
  ...
  public Object removeFirst() {
    ...
    lock.acquire();
    while (list.isEmpty()) listEmpty.sleep();
    o = list.removeFirst();
    lock.release();
  ...
  private static class PingTest implements Runnable {
    ...
    public void run() {
      for (int i = 0; i < 10; i++) pong.add(ping.removeFirst());
    }
