...
public class NachosSecurityManager extends SecurityManager {
  ...
  /**
   * Return a privilege object for this security manager. This security manager must not be the
   * active security manager.
   *
  ...
  private boolean isPrivileged() {
    ...
    if (!TCB.isNachosThread()) return true;
  ...
  /**
   * Check the specified permission. Some operations are permissible while not grading. These
   * operations are regulated here.
   *
  ...
  public void checkPermission(Permission perm) {
    ...
    if (perm instanceof RuntimePermission) {
      ...
      if (name.equals("createClassLoader")) no(perm);
    }
    ...
    if (!fullySecure) {
      if (perm instanceof NetPermission) {
        ...
        if (name.equals("specifyStreamHandler")) return;
      }
      ...
      if (perm instanceof AWTPermission) {
        // permit AWT stuff
        if (name.equals("accessEventQueue")) return;
      }
    }
      ...
      if (perm.getActions().equals("read")) {
        ...
        try {
          ...
          if (f.isFile()) {
            ...
            if (p != null) {
              if (p.equals(testDirectory)) no(perm);
            }
          }
        } catch (Throwable e) {
          rethrow(e);
        ...
        try {
          ...
          if (f.isFile()) {
            File p = f.getParentFile();
            if (p != null && p.equals(testDirectory)) return;
          }
        } catch (Throwable e) {
          no(perm);
        }
        ...
        if (Machine.networkLink() == null) no(perm);
      } else {
        no(perm);
      }
    }
  ...
  /**
   * Called by the <tt>java.lang.Thread</tt> constructor to determine a thread group for a child
   * thread of the current thread. The caller must be privileged in order to successfully create the
   * thread.
   *
   * @return a thread group for the new thread, or <tt>null</tt> to use the current thread's thread
   *     group.
   */
