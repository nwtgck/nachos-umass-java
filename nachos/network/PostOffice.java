...
/**
 * A collection of message queues, one for each local port. A <tt>PostOffice</tt> interacts directly
 * with the network hardware. Because of the network hardware, we are guaranteed that messages will
 * never be corrupted, but they might get lost.
 *
 * <p>The post office uses a "postal worker" thread to wait for messages to arrive from the network
 * and to place them in the appropriate queues. This cannot be done in the receive interrupt handler
 * because each queue (implemented with a <tt>SynchList</tt>) is protected by a lock.
 */
public class PostOffice {
  /**
   * Allocate a new post office, using an array of <tt>SynchList</tt>s. Register the interrupt
   * handlers with the network hardware and start the "postal worker" thread.
   */
  public PostOffice() {
    ...
    for (int i = 0; i < queues.length; i++) queues[i] = new SynchList();
  ...
  /** Called when a packet has arrived and can be dequeued from the network link. */
  ...
  public void send(MailMessage mail) {
    if (Lib.test(dbgNet)) System.out.println("sending mail: " + mail);
  ...
  /**
   * Called when a packet has been sent and another can be queued to the network link. Note that
   * this is called even if the previous packet was dropped.
   */
